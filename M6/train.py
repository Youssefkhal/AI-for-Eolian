...
class SlotAttentionDegradation(nn.Module):
    ...
    21 Slots -> 21 Steps (Per-Slot MLP, no LSTM):
    - Slot 1 = Initial stiffness values (KL, KR, KLR)
    - Slots 2-21 = Each slot directly predicts a drop via shared MLP
    - Physics: KL,KR drops <= 0 (degrade); KLR drops >= 0 (increases)
    ...
    def __init__(self, input_size=8, d_model=64, num_heads=4, num_slots=21,
        ...
        # Input embedding: 8 features -> d_model
        ...
        # 21 learnable slot vectors
        self.slots = nn.Parameter(torch.randn(1, num_slots, d_model) * 0.02)

        # Slot refinement layers (cross-attn, self-attn, MLP)
    ...
    def forward(self, x, seq_len=None):
        ...
        slots = self.slots.expand(batch_size, -1, -1)  # [B, 21, d_model]

        # Iterative slot refinement (x3)
        ...
        # Split: Slot 1 = initial, Slots 2-21 = drops
        slot_initial = slots[:, 0:1, :]   # [B, 1, d_model]
        slot_drops = slots[:, 1:, :]      # [B, 20, d_model]

        # Per-slot MLP predictions
        initial_decoded = self.initial_proj(slot_initial)  # [B, 1, 3]
        raw_drops = self.drop_proj(slot_drops)             # [B, 20, 3]

        # Physics constraint: KL, KR drops <= 0 (degrade); KLR drops >= 0 (increases)
        drops_kl_kr = -torch.abs(raw_drops[:, :, :2])   # KL, KR always decrease
        drops_klr = torch.abs(raw_drops[:, :, 2:3])     # KLR always increases
        drops = torch.cat([drops_kl_kr, drops_klr], dim=2)  # [B, 20, 3]
...
def train_model(X_train, Y_train, X_val, Y_val, epochs=3000, batch_size=8, lr=0.001):
    """Train the model with composite loss (tuned for per-slot MLP architecture)."""
