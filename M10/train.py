"""
M10: XAI-Enhanced SwiGLU Ψ-NN — Pile Stiffness Degradation
=============================================================
Extends M9 with XAI (Token Attribution, LRP, Attention Rollout).
...
Curve-based evaluation metrics (MAPE, R² per scenario/variable).
...
def evaluate_all(model, X_test, Y_test_orig, scaler_Y, label="Model"):
    ...
    # Curve-based metrics (per scenario, per variable, then averaged)
    curve_r2s = {vn: [] for vn in var_names}
    curve_mapes = {vn: [] for vn in var_names}
    for i in range(Y_test_orig.shape[0]):
        for vi, vn in enumerate(var_names):
            t, p = Y_test_orig[i, :, vi], pred_orig[i, :, vi]
            ss_res = np.sum((t - p)**2)
            ss_tot = np.sum((t - t.mean())**2)
            r2 = float(1 - ss_res / ss_tot) if ss_tot > 1e-20 else 1.0
            curve_r2s[vn].append(r2)
            mask = np.abs(t) > 1e-6
            mape = float(np.mean(np.abs(t[mask] - p[mask]) / np.abs(t[mask])) * 100) if mask.sum() > 0 else 0.0
            curve_mapes[vn].append(mape)
    metrics['curve_overall'] = {
        'r2': float(np.mean([np.mean(v) for v in curve_r2s.values()])),
        'mape': float(np.mean([np.mean(v) for v in curve_mapes.values()])),
    }
    metrics['curve_per_variable'] = {
        vn: {'r2': float(np.mean(curve_r2s[vn])), 'mape': float(np.mean(curve_mapes[vn]))}
        for vn in var_names
    }

    print(f"\n  {label}:")
    print(f"    Overall:  R²={m_overall['r2']:.4f}  Curve R²={metrics['curve_overall']['r2']:.4f}  MAPE={metrics['curve_overall']['mape']:.2f}%")
    for n in var_names:
        m = m_per_var[n]
        cm = metrics['curve_per_variable'][n]
        print(f"    {n:>3}:  R²={m['r2']:.4f}  Curve R²={cm['r2']:.4f}  MAPE={cm['mape']:.2f}%")
...
def main():
    print("=" * 60)
    print("M10: XAI-Enhanced SwiGLU Ψ-NN — Pile Stiffness Degradation")
    print("=" * 60)
    ...
    # Use curve-based R² in comparison for consistency with webapp
    def _make_comp(m, params, extra=None):
        entry = {
            'params': params,
            'overall': {**m['overall'], 'r2': m['curve_overall']['r2']},
            'per_variable': {
                vn: {**m['per_variable'][vn], 'r2': m['curve_per_variable'][vn]['r2']}
                for vn in ['KL', 'KR', 'KLR']
            },
        }
        if extra:
    ...
    comparison = {
        'teacher_m6':      _make_comp(teacher_metrics, n_teacher),
        'student_stage_a': _make_comp(student_metrics, n_student),
        'psi_model_m7':    _make_comp(psi_metrics, n_psi),
        'psi_model_m9':    _make_comp(psi_metrics, n_psi, {
            'k_star': k_star,
    ...
    print(f"\n{'='*60}")
    print("All M10 artifacts saved to", SCRIPT_DIR)
