"""
M8: Efficient Ψ-NN Pile Stiffness Degradation - Web Application
===============================================================
Shows Ψ-model predictions alongside structure discovery:
  - Prototype clustering, learned relation matrix, compression stats
...
class EfficientSlotMLP(nn.Module):
    """Bottleneck feed-forward matching train.py."""
...
class SlotAttentionPsiModel(nn.Module):
    """M8 Ψ-NN structured model (must match train.py exactly)."""
...
def load_all():
    ...
    try:
        ...
        # Curve-based overall metrics (across all scenarios)
        all_mapes, all_nrmses, all_r2s = [], [], []
        ...
        m_overall = {
            'curve_mape': round(float(np.mean(all_mapes)), 3),
            'curve_nrmse': round(float(np.mean(all_nrmses)), 3),
            'curve_r2': round(float(np.mean(all_r2s)), 6),
        }
        m_per_var = {}
        for vn in var_names:
            m_per_var[vn] = {
        ...
        # Per-step curve metrics (across all scenarios at each step)
        m_per_slot = []
        for s in range(max_seq_len):
            ...
            step_var = {}
            for vi, vn in enumerate(var_names):
                t_vals = Y_original[:, s, vi]
                p_vals = pred_original[:, s, vi]
                mask = np.abs(t_vals) > 1e-6
                if mask.sum() > 0:
                    step_mape = float(np.mean(np.abs(t_vals[mask] - p_vals[mask]) / np.abs(t_vals[mask])) * 100)
                else:
                    step_mape = 0.0
...
HTML = '''<!DOCTYPE html><html><head><title>M8 Psi-NN Pile Stiffness</title>
...
<div class="container">
<h1>M8: &Psi;-NN Pile Stiffness Degradation</h1>
<p class="subtitle">Efficient &Psi;-NN with Bottleneck MLP, Learnable R, Physics Loss &nbsp;|&nbsp; <a href="/diagnosis" style="color:#ffa500;text-decoration:underline">Worst-Scenario Diagnosis &rarr;</a></p>
...
<h2>Per-Step Curve MAPE (Across All Test Scenarios)</h2>
...
function renderOverall(metrics){
    ...
    const cm=scenarios.length?{mape:(scenarios.reduce((a,s)=>a+s.curve_metrics.avg_mape,0)/scenarios.length).toFixed(2),nrmse:(scenarios.reduce((a,s)=>a+s.curve_metrics.avg_nrmse,0)/scenarios.length).toFixed(2),r2:(scenarios.reduce((a,s)=>a+s.curve_metrics.avg_r2,0)/scenarios.length).toFixed(4)}:{mape:'--',nrmse:'--',r2:'--'};
...
DIAGNOSIS_HTML = r'''<!DOCTYPE html><html><head><title>M8 Diagnosis - Worst Scenarios</title>
<script src="https://cdn.jsdelivr.net/npm/chart.js@4.4.7/dist/chart.umd.min.js"></script>
<style>
*{margin:0;padding:0;box-sizing:border-box}
body{font-family:'Segoe UI',sans-serif;background:linear-gradient(135deg,#1a1a2e,#16213e);min-height:100vh;color:#fff;padding:20px}
...
<p class="subtitle">Curve-Based Error Analysis &mdash; M8 Efficient &Psi;-NN (45,486 params)</p>
...
if __name__ == '__main__':
    print("=" * 50)
    print("M8: Psi-NN Pile Stiffness Degradation")
    print("=" * 50)
    if load_all():
