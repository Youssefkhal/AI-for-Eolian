...
class SlotAttention(nn.Module):
    ...
    def forward(self, inputs: torch.Tensor, return_attn: bool = False) -> Tuple[torch.Tensor, Optional[torch.Tensor]]:
        n, t, d = inputs.shape
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, memory: torch.Tensor, tgt_mask: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
...
class SlotTransformer(nn.Module):
    def __init__(
        ...
        self.enc_layers = nn.ModuleList([EncoderLayer(d_model, n_heads, dropout) for _ in range(n_enc_layers)])
        self.dec_layers = nn.ModuleList([DecoderLayer(d_model, n_heads, dropout) for _ in range(n_dec_layers)])

        self.f0_in = nn.Linear(1, d_model)
        self.f0_out = nn.Linear(d_model, 1)
        self.mass_head = nn.Sequential(nn.Linear(10, 16), nn.ReLU(), nn.Linear(16, 3))
    ...
    def forward(
    ...
    ) -> Tuple[torch.Tensor, Dict[str, torch.Tensor], Optional[Dict[str, List[torch.Tensor]]]]:
        ...
        hn = _sym_nonneg(self.hn_head(slots[:, 1:, :]).view(n, self.n_yield, 3, 3))
        ...
        pred_f0 = self.f0_out(dec_out).squeeze(-1)

        mass_diag = F.softplus(self.mass_head(x_static)) + 1.0
        phys_f0 = _rayleigh_ritz_frequency(hij, mass_diag)

        aux = {"h0": h0, "hn": hn, "w": w, "hij": hij, "m_diag": mass_diag, "phys_f0": phys_f0}
        if not return_attn:
            return pred_f0, aux, None
